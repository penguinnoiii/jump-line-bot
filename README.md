...
3. Send **any message** first — a new student always hits the login gate before guidance: reply **"ยินยอม"**, then your phone number (e.g. `0812345678`), then the 6-digit OTP code (shown inline in mock mode), then full name, school, student ID, grade/room, and interest. After that, ask anything, e.g. *"อยากเข้าสายวิทย์-คณิต ต้องเตรียมตัวยังไงบ้าง"* — the bot replies with **personalized** guidance, addressing you by first name (no re-asking).

---

## Rich menu (tappable buttons)

The OA has a 4-button rich menu — **เข้าสู่ระบบ / ค้นหาโรงเรียน / แนะแนว / แดชบอร์ด** — so you can demo by tapping instead of typing. **เข้าสู่ระบบ** (Log in) always (re)starts the onboarding flow — it's a reset-command alias, so tapping it works whether or not you're already logged in, great for repeating the demo. **ค้นหาโรงเรียน**/**แนะแนว** send a message the bot already handles, but only reach the LLM once login is complete — tapped early, their text just answers whichever login question is current. **แดชบอร์ด** opens `/dashboard` (a `uri` action, not a message) — see below. AIS Number Verification/OTP re-verification is still available by typing outside the login flow (e.g. a phone number, or *"ขอ OTP ..."*).

- Image generator: `scripts/richmenu-image.js` (renders Thai via `@napi-rs/canvas`; supports any number of panels).
- Setup/refresh: `npm run richmenu` (creates the menu, uploads the image, sets it as default; clears old menus first). Re-run after changing labels or button actions. Set `APP_BASE_URL` if your deployment isn't `https://jump-line-bot.onrender.com`.
...
- `src/ais.js` — **AIS Open API** identity services for the *"ยืนยันตัวตน"* step:
  ...
  **Built into login, and linked to the student record:** phone verification (OTP) is now a required step of logging in itself — see `src/onboarding.js` above. A student can also re-verify a different number later (type a phone number, or *"ขอ OTP ..."*) once already logged in; that re-links onto their existing record live. Either way, the Teacher View always reflects current status: a ✅ **ยืนยันแล้ว** badge + the real number, or **ยังไม่ยืนยันเบอร์** otherwise.
