...
3. Send **any message** first — a new student always hits the login gate before guidance: reply **"ยินยอม"**, then full name, school, student ID, grade/room, and interest. After that, ask anything, e.g. *"อยากเข้าสายวิทย์-คณิต ต้องเตรียมตัวยังไงบ้าง"* — the bot replies with **personalized** guidance, addressing you by first name (no re-asking).

---

## Rich menu (tappable buttons)

The OA has a 3-button rich menu — **เข้าสู่ระบบ / ค้นหาโรงเรียน / แนะแนว** — so you can demo by tapping instead of typing. **เข้าสู่ระบบ** (Log in) always (re)starts the onboarding flow — it's a reset-command alias, so tapping it works whether or not you're already logged in, great for repeating the demo. The other two buttons send a message the bot already handles (school comparison, guidance), but only reach the LLM once login is complete — tapped early, their text just answers whichever login question is current. AIS Number Verification/OTP are still available, just by typing (e.g. a phone number, or *"ขอ OTP ..."*) rather than from the menu.
...
- `src/onboarding.js` — **student "log in" gate**: consent, then 5 general-info questions — **ชื่อ-นามสกุล (full name), โรงเรียน, รหัสนักเรียน, ชั้น/ห้อง, ความสนใจ/เป้าหมาย** — each skippable with **"ข้าม"**. The student is addressed by their **first name**, derived from the full name (not a separately-asked nickname). Declining consent still lets a student chat (generic, non-personalized) rather than blocking them outright. Send **"เข้าสู่ระบบ"** (or "แก้ไขข้อมูล") anytime to restart it. The profile feeds Typhoon so replies are genuinely tailored — e.g. opening with the student's stated goal, not a generic checklist. The very first message also gives a short **"what can Jump do"** list before asking to log in.
